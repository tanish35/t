    ...
    <div className="h-screen flex-1 overflow-y-auto">
      <div className="mx-auto w-full max-w-[1180px] px-5 py-8 sm:px-8">
        ...
        <section className="overflow-hidden rounded-[26px] border border-[var(--line)] bg-[var(--card)] shadow-[0_14px_32px_rgba(16,18,16,0.06)]">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-4 border-b border-[var(--line)] px-5 py-4 text-[11px] font-bold tracking-[0.12em] text-[var(--sea-ink-soft)] uppercase sm:grid-cols-[minmax(0,1fr)_110px_110px_32px] sm:px-6">
            <span>Project</span>
          ...
          <div className="divide-y divide-[var(--line)]">
            {projects.map((project) => (
              <Link
                ...
                className="group grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-5 py-5 no-underline hover:bg-[var(--link-bg-hover)] sm:grid-cols-[minmax(0,1fr)_110px_110px_32px] sm:px-6"
              >
                <span className="min-w-0">
                  <span className="block truncate text-[15px] font-bold text-[var(--sea-ink)]">
                    {project.name}
                  </span>
                  <span className="mt-1 block truncate text-[13px] text-[var(--sea-ink-soft)]">
                    {project.description || "No description"}
                  </span>
                </span>
                <span className="hidden text-sm font-semibold tabular-nums text-[var(--sea-ink)] sm:block">
